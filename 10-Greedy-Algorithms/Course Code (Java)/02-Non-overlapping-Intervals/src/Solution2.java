    ...
    public int eraseOverlapIntervals(Interval[] intervals) {
        ...
        Arrays.sort(intervals, new Comparator<Interval>() {
            ...
            public int compare(Interval o1, Interval o2) {
                if(o1.end != o2.end)
                    return o1.end - o2.end;    // 结束的不一致时，谁结束的早，谁排在前面
                return o1.start - o2.start;    // 谁的起始值小，谁排在前
            }
        });
        ...
        for(int i = 1 ; i < intervals.length ; i ++)
            if(intervals[i].start >= intervals[pre].end){   // 这种结构需要保存下来
                res ++;   // 保留的数组+1
                pre = i;
            }

        return intervals.length - res;  // 不保留的个数
    }
